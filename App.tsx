...
import { LayoutDashboard, GraduationCap, Bot, Settings, Database, Server, ChevronRight, AlertCircle, RefreshCw } from 'lucide-react';
    ...
    <div className="flex h-screen bg-slate-50 font-sans">
      ...
      <aside className="w-64 bg-white border-r border-slate-200 flex flex-col shrink-0 z-20 hidden md:flex">
        ...
        <nav className="flex-1 p-4 space-y-2">
          <button onClick={() => setCurrentView(ViewMode.DASHBOARD)} className={navItemClass(ViewMode.DASHBOARD)}>
            <LayoutDashboard size={20} />
            <span>Dashboard</span>
          </button>
          <button onClick={() => setCurrentView(ViewMode.STUDENTS)} className={navItemClass(ViewMode.STUDENTS)}>
            <GraduationCap size={20} />
            <span>Students</span>
          </button>
          <button onClick={() => setCurrentView(ViewMode.AI_SQL)} className={navItemClass(ViewMode.AI_SQL)}>
            <Bot size={20} />
            <span>AI Data Analyst</span>
          </button>
          <button onClick={() => setCurrentView(ViewMode.SETTINGS)} className={navItemClass(ViewMode.SETTINGS)}>
            <Settings size={20} />
            <span>Config</span>
          </button>
        </nav>
      ...
      <main className="flex-1 overflow-auto relative">
        <header className="sticky top-0 z-10 bg-white/80 backdrop-blur-sm border-b border-slate-200 px-8 py-4 flex justify-between items-center">
            <h1 className="text-xl font-bold text-slate-800">
                {currentView === ViewMode.DASHBOARD && 'Dashboard Overview'}
                {currentView === ViewMode.STUDENTS && 'Student Directory'}
                {currentView === ViewMode.AI_SQL && 'AI SQL Generator'}
                {currentView === ViewMode.SETTINGS && 'System Configuration'}
            </h1>
            <div className="flex items-center gap-4">
                <div className="text-right hidden sm:block">
                    <div className="text-sm font-medium text-slate-900">Admin User</div>
                    <div className="text-xs text-slate-500">School Administrator</div>
                </div>
        ...
        <div className="p-8">
            {loading ? (
                <div className="flex items-center justify-center h-64 text-indigo-600 flex-col gap-4">
